...
fn happy_path() {
    ...
    let clawback_amount = 123782;
    contract.clawback(&admin, &clawback_amount);

    assert_eq!(
        contract.get_remaining_amount(),
        (test_data::TOTAL_AMOUNT - test_data::ACCOUNT_1_ALLOCATION - clawback_amount)
    );
    assert_eq!(token_contract.balance(&admin), (clawback_amount));
}
...
fn error_airdrop_not_expired() {
    ...
    assert_eq!(
        contract.try_clawback(&admin, &10),
